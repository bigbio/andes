...
This is the full reference for the `cimas` binary and its outputs. For a quick start and benchmark summary, see [`README.md`](README.md). For porting Java MS-GF+ command lines and numeric legacy flags, see [`docs/CLI_MIGRATION.md`](docs/CLI_MIGRATION.md).

Run `cimas --help` for auto-generated help derived from the same `Cli` struct documented below.
...
8. [Java MS-GF+ → cimas migration](#8-java-ms-gf--cimas-migration)
...
All flags use kebab-case long options (`--flag-name`). Several flags also accept legacy Java MS-GF+ numeric values (see §8). The CLI is implemented in `crates/cimas/src/bin/cimas.rs`.
...
cimas reads the same modification file format as Java MS-GF+. The parser lives in `crates/model/src/modification.rs` and `crates/model/src/aa_set.rs`.
...
| Field | Rule |
|---|---|
| `<mass>` | Numeric monoisotopic mass delta in Da. Composition strings (`C2H3N1O1`) are **not** supported in cimas. |
...
When `--mods` is omitted, cimas uses these two modifications as built-in defaults.
...
cimas writes Percolator `.pin` (always) and optionally `.tsv`. Implementation: `crates/output/src/pin.rs`, `crates/output/src/tsv.rs`.
...
For **mzML** inputs when `--fragmentation auto` (the default), cimas peeks the input file before loading the full dataset:
...
A `.raw` file carries the activation method and analyzer in vendor metadata, so cimas reads them directly (no mzML peek) and routes through the same parquet-store selection as mzML — e.g. beam-type CID (HCD) on an Orbitrap → `hcd_qexactive_tryp`. `--protocol` from the CLI still selects protocol-specific models (`tmt`, `itraq`); explicit `--fragmentation`/`--instrument` are not required.
...
| CV accession | Name (PSI-MS) | cimas method | Notes |
|---|---|---|---|
...
cd cimas
cargo build --release
# Binary: target/release/cimas   (mzML + MGF; pure Rust)
```
...
RUSTUP_TOOLCHAIN=stable cargo build --release -p cimas --features thermo
...
RUSTUP_TOOLCHAIN=stable cargo build --release -p cimas --features "thermo timstof"
```
...
cimas includes a native Rust training engine — **`cimas train`** — that generates scoring models from your own data and writes them into the same Parquet model store the bundled models live in. No Java `ScoringParamGen` round-trip is needed.

Training is **bootstrap-supervised**: cimas searches your spectra with a seed model, keeps the confident PSMs (target-decoy q ≤ `--train-fdr`), and re-estimates the per-partition rank and mass-error distributions from them. Trained models are auto-selected by instrument/protocol at search time, and the store supports incremental add / remove / reweight / decay updates with a held-out acceptance gate.
...
```bash
cimas --spectrum more.mzML --database mydata.fasta --output-pin out.pin \
...
## 8. Java MS-GF+ → cimas migration

cimas accepts **both** canonical kebab-case flags with named enum values **and** legacy Java short flags / numeric IDs. Existing quantms scripts using `--fragmentation 3 --instrument 3 --protocol 4` continue to work.

### 8a. Flag rename table

| Java MS-GF+ | cimas canonical | cimas legacy alias |
|---|---|---|
...
shift → tightened precursor tolerance) before the main search. cimas ports this
...
If you use cimas in published work, cite the original MS-GF+ paper:

...
<sub>**Metric.** Counts are Percolator `q ≤ 0.01` under one uniform methodology (plain FASTA + andes `XXX_` decoys + Percolator 3.7.1 `--seed 42 -Y`) — that is what makes the rows comparable. They are **not** entrapment-validated: a 2026-09-04 audit found the Astral database has no entrapment component and the UPS1 one is not 1:1, so the nominal 1% there is really ~2.6–3.6% true FDP. These four counts also predate the current default and are not traceable to a recorded run — treat them as historical. **‡** Wall times measured 2026-09-04 on merged `main` (8 threads, one host); competitor timings were not re-run, so no current speed ratio against them is claimed. andes remains ~10–40x faster than Java MS-GF+. **†** Java Astral at q≤1%. N=1 per dataset; Astral on converted mzML. Method, caveats and current measurements: [`docs/benchmarks/`](docs/benchmarks/README.md).</sub>

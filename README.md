...
Benchmarked at 1% FDR across three reference datasets. **Read the metric note below the table before quoting these numbers** — an audit on 2026-09-04 found the entrapment description that used to sit here overstated what two of the three columns measure ([`2026-09-04-tree-count-default-and-entrapment-audit.md`](docs/benchmarks/2026-09-04-tree-count-default-and-entrapment-audit.md)):
...
<sub>**Metric, corrected 2026-09-04.** These counts are at Percolator `q ≤ 0.01` under one uniform methodology (plain FASTA + andes `XXX_` decoys + Percolator 3.7.1 `--seed 42 -Y`), which is what makes the rows comparable to each other. They were previously described as "1% true entrapment-FDP against a 1:1 entrapment database"; measurement shows that claim does not hold for two of the three columns. **Astral**: the benchmark database (`hye.fasta`, 31,889 seqs) contains **no entrapment sequences**, so a re-run returns 0 entrapment hits and no entrapment FDP is computable — that column is not entrapment-validated. (A genuine 1:1 entrapment cross-check on Astral was done separately in [`2026-06-15`](docs/benchmarks/2026-06-15-public-benchmark.md) with a purpose-built database.) **UPS1**: `yeast_entrap.fasta` is 6,733 target to 4,531 entrapment, **not 1:1**, so `FDP = 2·ENT/total` is the wrong estimator; the correct `ENT/total × (1 + T/E)` puts the nominal 1% at **2.6% true FDP** (protein-count basis) or **3.6%** (searchable-peptide basis, the more defensible one). Cross-engine and cross-configuration comparisons are unaffected — every row carries identical scaling — but absolute "1% true FDP" claims are not supported for those two columns. **Provenance gap:** the four counts above do not appear in any document in [`docs/benchmarks/`](docs/benchmarks/); the closest recorded run ([`2026-06-26-owngeometry-ab.md`](docs/benchmarks/2026-06-26-owngeometry-ab.md)) reports 36,730 / 11,215 / 14,919. They are flagged here rather than restated as verified or quietly removed, and should be re-derived from a recorded run. **andes beats Comet on all three datasets** and beats Java MS-GF+ on Astral and TMT; **on UPS1 (low-res LFQ) andes leads Comet but trails Java MS-GF+ by ~5%**, Java's strongest regime and a known limitation of andes's rank model — stated plainly rather than hidden. **Speed:** measured 2026-09-04 on merged `main` (`1b8520f8`), same host and dataset with the tree-count default as the only variable, Astral runs in **244 s vs 400 s** under the previous default — **1.64x faster for −8 PSMs (−0.02%)**; UPS1 runs in 50 s. The earlier "450 s vs Comet's 217 s" comparison predates both this default and that binary, and Comet is no longer installed on the benchmark host, so **no refreshed andes-vs-Comet timing is claimed**. andes remains ~10–40x faster than Java MS-GF+, which finishes in 9 min – 2.5 h. Opt-in `--chimeric` recovers co-isolated second peptides for a further gain on high-res. **†** Java Astral shown at q≤1%. N=1 run per dataset; the Astral run is on converted mzML. See [`docs/benchmarks/`](docs/benchmarks/).</sub>
...
- **FDR honesty:** counts are at Percolator `q ≤ 0.01` under one methodology for every engine. Where an entrapment database exists the true FDP is measured alongside and reported; see the metric note under the table and [`2026-09-04-tree-count-default-and-entrapment-audit.md`](docs/benchmarks/2026-09-04-tree-count-default-and-entrapment-audit.md) for which datasets that covers and which it does not.
...
[`docs/benchmarks/2026-08-28-config-matrix.md`](docs/benchmarks/2026-08-28-config-matrix.md).

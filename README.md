...
| a comparison search engine 0.14.7 | 32,091 | 11,232 | 15,653 |
...
- **Engines:** andes (this repo), Java MS-GF+ [v20240326](https://github.com/MSGFPlus/msgfplus/releases/tag/v2024.03.26), a comparison search engine 0.14.7, a comparison engine 2025.01 (via OpenMS), ProSE (OpenMS). Parameters harmonized per dataset (trypsin, ≤2 missed cleavages, matched fixed/variable mods and precursor/fragment tolerances).
...
- **PIN building:** andes / a comparison search engine / a comparison engine write Percolator PIN directly; Java MS-GF+ via `MzIDToTsv` + `build_pins.py` (its concatenated-TDA mzid crashes `msgf2pin`); ProSE via OpenMS → idXML → `build_pins.py` (ProSE caps fragment tolerance at 0.1 Da, used on the low-res sets).

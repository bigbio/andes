...
LFQ_MGF="${LFQ_MGF:-$BENCH/PXD001819/sample.mgf}"
LFQ_FASTA="${LFQ_FASTA:-$BENCH/PXD001819/human-uniprot-contaminants.fasta}"
LFQ_PARAM="${LFQ_PARAM:-HCD_QExactive_Tryp.param}"

ASTRAL_MZML="${ASTRAL_MZML:-$BENCH/astral/sample.mzML}"
ASTRAL_FASTA="${ASTRAL_FASTA:-$BENCH/astral/human.fasta}"
ASTRAL_PARAM="${ASTRAL_PARAM:-HCD_HighRes_Tryp.param}"

TMT_MGF="${TMT_MGF:-$BENCH/tmt/sample.mgf}"
TMT_FASTA="${TMT_FASTA:-$BENCH/tmt/uniprot.fasta}"

...
/// Default weight for the PARTIAL-GLYCAN b/y evidence term (idea B). Scales
/// `partial_glycan_by` so the true LARGE/HIGH-CHARGE backbone — whose bare b/y is too
/// thin for gp to pick — can win the collapse on this SEQUENCE-specific evidence.
/// Unlike the learned selector (re-ranking weak evidence), this selects on ADDED real
/// evidence. Overridable via `ANDES_GLYCO_GP_P`; default 0.0 (opt-in until @1%-tuned).
pub const GLYCO_GP_P_DEFAULT: f32 = 0.0;
...
/// Process-constant P (partial-glycan b/y evidence weight); same read-once contract.
pub fn glyco_gp_p() -> f32 {
    gp_weight_from_env("ANDES_GLYCO_GP_P", GLYCO_GP_P_DEFAULT)
}
...
pub fn glyco_gp_fused_score(
    ...
    core_y_hits: f32,
    hyperscore: f32,
    partial_glycan_by: f32,
    k: f32,
    j: f32,
    h: f32,
    p: f32,
) -> f32 {
    rank + k * ladder + j * core_y_hits + h * hyperscore + p * partial_glycan_by
}
...
mod tests {
    ...
    fn gp_fused_score_lets_rank_rescue_truth_but_a_large_ladder_still_wins() {
        ...
        let (k, j, h, p) = (GLYCO_GP_K_DEFAULT, 0.0, 0.0, 0.0); // isolate rank/ladder
        let truth = glyco_gp_fused_score(15.0, 0.05, 0.0, 0.0, 0.0, k, j, h, p); // 15 + 2.5 = 17.5
        let wrong = glyco_gp_fused_score(2.0, 0.06, 0.0, 0.0, 0.0, k, j, h, p); //  2 + 3.0 =  5.0
        ...
        let strong_glycan = glyco_gp_fused_score(2.0, 1.0, 0.0, 0.0, 0.0, k, j, h, p); // 2 + 50 = 52
    ...
    fn gp2_core_y_hit_count_term_rescues_coverage_strong_truth() {
        ...
        let (k, j, h, p) = (GLYCO_GP_K_DEFAULT, GLYCO_GP_J_DEFAULT, 0.0, 0.0); // 50, 5, no hyperscore/partial
        // truth: rank 8, ladder 0.1, core-Y 6 ; winner: rank 12, ladder 0.1, core-Y 2
        let truth = glyco_gp_fused_score(8.0, 0.1, 6.0, 0.0, 0.0, k, j, h, p); // 8 + 5 + 30 = 43
        let winner = glyco_gp_fused_score(12.0, 0.1, 2.0, 0.0, 0.0, k, j, h, p); // 12 + 5 + 10 = 27
        ...
        assert!(glyco_gp_fused_score(8.0, 0.1, 6.0, 0.0, 0.0, k, 0.0, h, p)
            < glyco_gp_fused_score(12.0, 0.1, 2.0, 0.0, 0.0, k, 0.0, h, p));
    }
    ...
    fn glyco_gp_weights_default_and_fusion_is_monotone() {
        ...
        assert_eq!(GLYCO_GP_P_DEFAULT, 0.0);
        // Monotone in each term (rank, ladder, core-Y, hyperscore, partial-glycan).
        let f = |rk, yl, cy, hs, pg| glyco_gp_fused_score(rk, yl, cy, hs, pg, 50.0, 5.0, 1.0, 1.0);

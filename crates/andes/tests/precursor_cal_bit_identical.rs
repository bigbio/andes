...
//! were generated by this branch's binary on this fixture. Note: `test.mgf`
//! is metadata-less MGF input, so under decision E the no-flags default model
//! is now `cid_lowres_tryp` (was `hcd_qexactive_tryp` before the
//! `--instrument` removal); the goldens were regenerated accordingly. To
//! regenerate after an intentional off-path change, run:
//!
//! ```text
...
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR points at crates/andes at test compile time.

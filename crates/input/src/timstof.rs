...
//!
//! Wraps the pure-Rust [`timsrust`] crate (the same reader a comparison search engine uses) and
...
impl TimsTofReader {
    ...
    fn convert(raw: &timsrust::Spectrum) -> Option<Spectrum> {
        ...
        Some(Spectrum {
            ...
            precursor_intensity,
            precursor_charge,
            // `timsrust` retention time is in SECONDS (a comparison search engine divides it by 60 to

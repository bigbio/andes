...
fn separable_18(n: usize, seed: u64) -> Dataset {
    ...
    for r in 0..n {
        for f in 0..NF { x[r * NF + f] = next(&mut s); }
...
fn trained_model_roundtrips_through_blob_and_separates() {
    ...
    assert_eq!(back, model, "model must survive blob round-trip byte-for-byte");

    // Separation holds through the decoded model.
    let mut hi = vec![0.0f32; 18]; hi[0] = 0.95;
    let mut lo = vec![0.0f32; 18]; lo[0] = 0.05;
    assert!(back.predict_logit(&hi) > back.predict_logit(&lo) + 1.0,
        "decoded model must separate signal from noise: hi={} lo={}",
        back.predict_logit(&hi), back.predict_logit(&lo));
}

...
fn bsa_generates_reasonable_candidate_count() {
    let target =
        FastaReader::load_all(BufReader::new(File::open(fasta("BSA.fasta")).unwrap())).unwrap();
    ...
    assert!(
        candidates.len() > 50,
        "got {} candidates, expected > 50",
        candidates.len()
    );
    assert!(
        candidates.len() < 50_000,
        "got {} candidates, expected < 50,000",
        candidates.len()
    );
    ...
    for c in &candidates {
        ...
        assert!(
            c.protein_index < 2,
            "BSA has only 2 proteins (target+decoy)"
        );
    }
...
fn tryp_pig_bov_generates_more_candidates_than_bsa() {
    ...
    let tpb_target = FastaReader::load_all(BufReader::new(
        File::open(fasta("Tryp_Pig_Bov.fasta")).unwrap(),
    ))
    .unwrap();

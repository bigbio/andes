# Migrating to cimas from Java MS-GF+

cimas accepts both the canonical Rust-idiomatic CLI form (named values, kebab-case) and the legacy Java MS-GF+ form (numeric IDs and short flag names) silently — running scripts written against Java MS-GF+ unchanged is supported.

This page is a quick-reference for porting commands. For the full CLI reference, see [`DOCS.md`](DOCS.md) §1.

## Table A — Java MS-GF+ flag → cimas flag
...
**cimas (legacy-form, drop-in for existing quantms scripts):**

The Java-style flags above don't translate verbatim — `-s`, `-d`, `-o` are Java-only. But the search-parameter flags do; for example, an existing quantms script that calls cimas with `--fragmentation 3 --instrument 3 --protocol 4` keeps working unchanged.
...
- `-tda 1` (target+decoy database analysis) is always on in cimas — decoys are generated by reversing target sequences at search time. The decoy prefix is configurable via `--decoy-prefix` (default `XXX_`).

...
Single-variable A/B (baseline vs `--rs3`, identical params, bundled-store model auto-selection, Percolator q≤0.01, on `bench-vm`, results in `/data/msgf-bench/rs3ab/`). Rs3 columns verified populated in `--rs3` arms, exactly 0 in baseline → clean A/B, Percolator genuinely saw the features.

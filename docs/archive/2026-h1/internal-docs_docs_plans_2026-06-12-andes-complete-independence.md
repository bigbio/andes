...
**Tech Stack:** Rust workspace (`crates/scoring`, `crates/model-train`, `crates/andes`); HPC SLURM for harvest/train; VM for benchmark; Percolator 3.7.1; PRIDE public datasets harvested with pridepy + MSFragger + own combined-FDR + own trainer.
...
**Gated procedure (uses the harvest/train infra on HPC). Not TDD.**

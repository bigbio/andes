...
snapshot). Two stages, HPC → VM:
1. **HPC — gold-standard PSMs:** run **MSFragger ∪ Comet → Percolator
   ...
   `/cluster/andes-training`. Access via the **`hpc-cluster` skill**. Hand the
   gold PSMs to the VM.
2. **VM — train andes on those labels:** `andes train` with the HPC gold PSMs as
...
0a. **HPC:** MSFragger → Percolator on the public corpus → gold-standard PSMs
    (training labels). *(Long pole; gates everything.)* Hand off to the VM.
0b. **VM:** `andes train` on the HPC gold PSMs → own-data `models.parquet`
...
1. **Native Rust inference.** A model's *inference path in andes* must be pure
   ...
   on HPC gold PSMs); only **inference** must be native Rust. Today that's the
...
- **HPC cluster = gold-standard PSM generation (training labels).** Run
  **MSFragger ∪ Comet → Percolator (consensus)** on big *public* datasets, fast,
  ...
  label set, not any andes code/model.) Access via the `hpc-cluster` skill;
  staging at `/cluster/andes-training`.
- **VM = training + benchmarking.** (a) **Train** andes models with `andes train`
  using the HPC gold-standard PSMs as the confident-label set → an own-data
...
- **This machine (local) = code improvements.** After a VM benchmark yields a
  ...
  it dispatches compute to HPC/VM and reads results back.)
- **Datasets (the actual staged VM benchmark set, per `reference_andes_infra_layout`):**
  ...
  CID-TMT) — each with its FASTA + numeric mods under `/data/msgf-bench/`.
...
Pacing: each iteration is real compute (search + Percolator = minutes–tens of minutes; gold-PSM generation on HPC + andes training on the VM = longer). Use long/dynamic `/loop` intervals tied to job completion, not tight polling.

## Starter backlog (across all three axes)

**PSMs — TMT/low-res (the lever: the model):**
- Train per-protocol models **on the VM from HPC-generated gold PSMs**: a dedicated **low-res CID-TMT** model (the known divergence) — vary peak-rank distribution, noise-model shape, segment count, training corpus. Benchmark each variant; keep the FDP-clean winner. (Model store + `andes train` machinery already exist; loss-table serialization too.)

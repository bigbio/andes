...
(`bench-host:/data/msgf-bench`) and getting a valid PSM@1% count via
...
setsid bash -c "cd /data/msgf-bench; java -Xmx16g -jar $JAR \
...
```bash
D=/data/msgf-bench/tmt-data; PIMG=quay.io/biocontainers/percolator:3.7.1--h3b5f4bd_2
...
## 7. Disk + hygiene (`/data` is only 100 GB)
- **`df -h /data` FIRST.** At >97% full, searches fail *silently* (empty logs).

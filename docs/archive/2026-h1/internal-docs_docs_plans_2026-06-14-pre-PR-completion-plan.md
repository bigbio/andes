...
datasets** (HPC-harvested, MSFragger-searched) **and fix every deferred hardening issue**.
**Only glyco stays parked.** This plan is grounded in an actual HPC recon (datasets, models,
raw availability) — not hand-waving.

## 0. Grounding facts (from HPC recon, 2026-06-14)
...
- **Raw spectra NOT kept on HPC** (deleted post-harvest) → benchmarking re-downloads the raw.
...
### 1a. Train the non-tryptic own models (HPC; flats already exist)
...
Ship the new model stores HPC→local→VM. **Verify provenance = own (train-from-msnet).**
...
| **H1** profile-mode mzML | Detect `MS:1000128` (profile); **centroid** in `build_peaks` (local-max + intensity-weighted) or fail loud. | **Creatable**: convert a `.raw` with ThermoRawFileParser `--noPeakPicking` → profile mzML; assert PSMs recover to ~centroided level. (qe-holdout HeLa.raw on HPC is a source.) |
...
2. **W1a** train non-tryptic models (HPC) → ship.
...
- H5/H6 still need datasets that aren't staged (HCD-ion-trap, EThcD) — Workstream 1's harvest
  pipeline produces them, but it's real HPC compute + download time. If a dataset can't be

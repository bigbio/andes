...
- **VM** (`bench-vm`) = benchmark cache. Its repo/models are STALE until re-verified against a commit.
- **HPC** = train cache. `andes-bin` builds from the REMOTE branch `feat/enzyme-support`; unpushed local

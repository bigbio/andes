...
**Architecture:** The rank-LLR core stays (the natural low-res advantage). A LightGBM binary classifier scores each observed peak `P(signal | peptide-agnostic features)`, is isotonic-calibrated offline on HPC, transcoded to a flat struct-of-arrays tree table, and evaluated by a hand-rolled zero-native-dep Rust walker. Its calibrated probability becomes a **new additive LLR term** `log(s/(1−s))` per matched fragment, computed once per peak at spectrum-prep and cached by intensity rank, so the GBDT never runs in the inner candidate loop. When a model carries no GBDT blob the scoring path is byte-identical to today. Training/data happen on HPC; benchmarking on the VM.

**Tech Stack:** Rust (workspace crates `scoring`, `model-train`, `andes`; Arrow/Parquet 53 already present — **zero new native deps**), Python 3 + LightGBM + scikit-learn `IsotonicRegression` (HPC anaconda only, offline), SLURM (HPC), Percolator 3.7.1 + entrapment-FDP scripts (VM).

**Source of truth:** spec at `internal-docs/specs/2026-06-15-andes-stronger-models-public-release-design.md`. Repo: `/Users/user/work/msgfplus-workspace/msgf-rust`. This plan lives in `internal-docs/` (Claude planning docs never ship in the public repo). Training **code** (Python pipeline) DOES live in the repo — it is part of the reproducible tool, documented by `TRAIN.md`.

**Two-host split:** HPC = generate data + train models. VM = benchmark only. See `reference_andes_infra_layout` and the `hpc-cluster` skill for access patterns (`become <group>`, base64-ship scripts, writes only on compute nodes, tag `--comment=andes:gbdt`).
...
- Create: `/Users/user/work/msgfplus-workspace/internal-docs/model-archive/msgf-derived-seed.parquet`

- [ ] **Step 1: Create the archive directory and copy the seed store**

```bash
mkdir -p /Users/user/work/msgfplus-workspace/internal-docs/model-archive
cp /Users/user/work/msgfplus-workspace/msgf-rust/resources/ionstat/models.parquet \
   /Users/user/work/msgfplus-workspace/internal-docs/model-archive/msgf-derived-seed.parquet
```

- [ ] **Step 2: Verify the copy is byte-identical and record provenance**

```bash
cmp /Users/user/work/msgfplus-workspace/msgf-rust/resources/ionstat/models.parquet \
    /Users/user/work/msgfplus-workspace/internal-docs/model-archive/msgf-derived-seed.parquet \
  && echo "ARCHIVE OK"
```
...
```bash
cd /Users/user/work/msgfplus-workspace/msgf-rust
...
```
(On Linux/HPC drop the empty `''` after `-i`.)
...
## Phase A3 — source-agnostic harvest + reproducible training (HPC, REMOTE)

**Surface to the user — runs on HPC with real data (per the hpc-cluster skill);
...
### Task A3.2: Per-slug clean retrain on HPC
Build `andes` on HPC (rsync `crates/`+`Cargo.{toml,lock}`; the VM/HPC repos are
...
- A2 is genuine research: feature design, label quality (co-isolation), calibration, and beating the field will iterate across sessions on HPC+VM. The A4 gate may take several rounds.

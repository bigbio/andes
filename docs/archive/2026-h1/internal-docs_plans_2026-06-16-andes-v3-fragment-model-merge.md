...
- [ ] **Step 1:** Wait for the Java-Astral run (`b6qav62q3`) to free the VM. Then re-sync the VM repo to committed HEAD **whole-tree** (kill drift): from Mac, `git -C msgf-rust archive HEAD | ssh bench-host 'tar -x -C /data/msgf-bench/repo/msgf-rust'` (or `git fetch+checkout` if the VM repo is a clone). Record `git rev-parse --short HEAD`.

...
**Tech Stack:** Rust (crates `scoring`, `model-train`, `andes`); existing zero-dep GBDT SoA walker (`gbdt_eval.rs`) + histogram regression trees (`gbdt/tree.rs`); Parquet model store; Percolator 3.7.1 for FDR; HPC (train) + VM (bench).
...
- [ ] **Step 1:** Wait for the Java-Astral run (`b6qav62q3`) to free the VM. Then re-sync the VM repo to committed HEAD **whole-tree** (kill drift): from Mac, `git -C msgf-rust archive HEAD | ssh bench-vm 'tar -x -C /data/msgf-bench/repo/msgf-rust'` (or `git fetch+checkout` if the VM repo is a clone). Record `git rev-parse --short HEAD`.
- [ ] **Step 2:** Rebuild: `ssh bench-vm 'cd <repo> && cargo +1.95.0 build --release -p andes --features thermo'`.
...
## Task 7: Train per-slug on HPC + A/B gate on the VM

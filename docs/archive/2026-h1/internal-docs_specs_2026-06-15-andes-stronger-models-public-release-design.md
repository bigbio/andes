...
5. **HPC = training/data; VM = benchmarking.**
...
lose). Retrain cleanly on more HPC-harvested gold PSMs (MSFragger∪Comet→Percolator — an
...
- **Training (HPC):** negative-undersample to ~4:1; optimize PR-AUC; **isotonic
...
### A3 · Clean `andes train` pipeline (HPC, documented)
...
- **A2 is genuine research** — feature design, label quality (co-isolation), calibration,
  and beating the field will iterate on HPC+VM across multiple sessions. The gate (A4)

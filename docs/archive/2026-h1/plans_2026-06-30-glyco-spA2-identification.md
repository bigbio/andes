...
## Build sequence
Phase 1 (sequon+glyco_psm) → `cargo test -p andes-glyco`. Phase 2 (glyco_search) → `cargo test -p andes`. Phase 3 (glyco_pin) → test. Phase 4 (CLI + smoke on staged mzML) → build + local glyco PIN + andes glyco-PSM count. Phase 5 (HPC comparative) → gate.

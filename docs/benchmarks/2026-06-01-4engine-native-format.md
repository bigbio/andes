...
Head-to-head of **Java MS-GF+**, **Sage**, **MSFragger**, and **andes** on two
...
1. **andes leads on PSMs *and* peptides on both datasets.**
   ...
   **+27% / +21%** vs MSFragger.
2. **Native `.raw` reading is exclusive to andes.** On this host Sage read
...
3. **Chimeric ~doubles confident IDs.** andes's two-pass cascade and
   ...
   survive FDR.
4. **Speed.** Sage is fastest on both datasets; andes is mid-pack; **Java
...
> All four engines search the **target-only** FASTA and add their own reversed decoys (Java `-tda 1`, Sage `generate_decoys`, andes auto) — except MSFragger, which needs a pre-built `rev_` target+decoy FASTA (see §4).
...
| Fragment tolerance | **20 ppm** (Sage/MSFragger; Java & andes are model-based) |
| Charge range | **2–4** |
...
| Threads | **8** |
| Chimeric (Astral) | andes `--chimeric`; MSFragger DDA+ `output_report_topN 2`; Sage `chimera: true` + `report_psms: 5` |

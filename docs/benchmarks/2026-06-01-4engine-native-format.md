...
> **Headline:** andes returns the most PSMs *and* the most distinct peptides
> at 1% FDR on **both** datasets, and is the **only** engine that reads Thermo
> `.raw` natively (a comparison search engine and the reference engine fall back to mzML; Java reads neither
> `.raw` nor `.d`).
...
|---|---|---:|---:|---:|---:|
| **andes** (`--chimeric`) | **`.raw` (native)** | 6:40 | 10.5 GB | **77,859** | **27,637** |
| the reference engine (DDA+, topN 2) | mzML | 4:27 | 23 GB | 73,909 | 25,600 |
| Java MS-GF+ | mzML | 2:26:49 | 6.0 GB | 33,425 | 22,050 |
| a comparison search engine (chimera, report_psms 5) | mzML | 1:48 | 8.0 GB | 32,091 | 21,397 |

### timsTOF — Bruker timsTOF DDA-PASEF, HeLa (`HeLa_IAA_F51_1.d`), non-chimeric

| Engine | Input | Wall time | Peak RAM | PSMs @1% | Peptides @1% |
|---|---|---:|---:|---:|---:|
| **andes** | **`.d` (native)** | 2:57 | 9.4 GB | **4,345** | **1,418** |
| a comparison search engine | `.d` (native) | 0:41 | 5.7 GB | 3,607 | 1,210 |
| the reference engine | `.d` (native) | 1:00 | 7.6 GB | 3,423 | 1,170 |
...
1. **andes leads on PSMs *and* peptides on both datasets.**
   Astral: **+5.3% PSMs / +8.0% peptides** vs the reference engine DDA+, and ~**2.3×** vs the
   non-chimeric Java/a comparison search engine. timsTOF: **+20% PSMs / +17% peptides** vs a comparison search engine,
   **+27% / +21%** vs the reference engine.
2. **Native `.raw` reading is exclusive to andes.** On this host a comparison search engine read
   **0 spectra** from the `.raw` and the reference engine aborted (`Could not find Batmass-IO
   ...
   natively.
3. **Chimeric ~doubles confident IDs.** andes's two-pass cascade and
   the reference engine's DDA+ both roughly double the non-chimeric engines on Astral.
   a comparison search engine's `chimera` flag added almost nothing here (+1.4% even at
   `report_psms: 5`): it emits many more PSMs per spectrum, but they do not
   survive FDR.
4. **Speed.** a comparison search engine is fastest on both datasets; andes is mid-pack; **Java
   MS-GF+ is ~22× slower than andes on Astral** (2.5 h vs 6:40). andes
   uses less than half of the reference engine's peak memory on Astral (10.5 GB vs 23 GB).
5. **The andes counts are method-robust.** Across Percolator mix-max, `-Y`
...
| a comparison search engine | v0.14.7 |
| the reference engine | 4.2 |
| andes | `dev` (2026-06-01 build, `--features "thermo timstof"`) |
...
- mzML (needed for Java/a comparison search engine/the reference engine, which don't read Thermo `.raw`): convert the `.raw` with [ThermoRawFileParser](https://github.com/compomics/ThermoRawFileParser) (`ThermoRawFileParser.sh -i <file>.raw -f 2`).
...
> All four engines search the **target-only** FASTA and add their own reversed decoys (Java `-tda 1`, a comparison search engine `generate_decoys`, andes auto) — except the reference engine, which needs a pre-built `rev_` target+decoy FASTA (see §4).
...
| Fragment tolerance | **20 ppm** (a comparison search engine/the reference engine; Java & andes are model-based) |
| Charge range | **2–4** |
| Enzyme / specificity | **Trypsin, fully specific (NTT = 2)** |
| Missed cleavages | **≤ 2** |
| Peptide length | **6–40** (a comparison search engine/the reference engine peptide mass 500–5000 Da) |
| Min peaks | **10** |
...
| Threads | **8** |
| Chimeric (Astral) | andes `--chimeric`; the reference engine DDA+ `output_report_topN 2`; a comparison search engine `chimera: true` + `report_psms: 5` |
...
**andes — Astral** (native `.raw`, chimeric):
```bash
andes --spectrum astral.raw --database HYE.fasta --output-pin rust.pin --mods mods.txt \
...
**andes — timsTOF** (native `.d`):
```bash
andes --spectrum HeLa.d --database human_yeast.fasta --output-pin rust.pin --mods mods.txt \
  --enzyme-specificity fully --max-missed-cleavages 2 --min-peaks 10 \
  --min-length 6 --max-length 40 --charge-min 2 --charge-max 4 --threads 8 \
...
**a comparison search engine** (mzML for Astral — a comparison search engine cannot read `.raw`; native `.d` for timsTOF):
```bash
a comparison search engine configs/a comparison search engine-astral.json  --write-pin -o out astral.mzML
a comparison search engine configs/a comparison search engine-timstof.json --write-pin -o out HeLa.d
```

**the reference engine** (needs a target+decoy FASTA — it does not generate decoys at search time):
```bash
java -Xmx24g -jar the reference engine-4.2.jar configs/the reference engine-astral.params  astral.mzML
java -Xmx7g  -jar the reference engine-4.2.jar configs/the reference engine-timstof.params HeLa.d
```

Config files: [`configs/a comparison search engine-astral.json`](configs/a comparison search engine-astral.json),
[`configs/a comparison search engine-timstof.json`](configs/a comparison search engine-timstof.json),
[`configs/the reference engine-astral.params`](configs/the reference engine-astral.params),
[`configs/the reference engine-timstof.params`](configs/the reference engine-timstof.params),
[`configs/mods.txt`](configs/mods.txt) (the andes / Java mods file).
...
because Percolator's automatic mode selection differs by engine. andes emits
...
the andes pins were additionally re-scored with (a) auto mix-max and (b) a
...
- **Input format.** andes searches the vendor-native `.raw`/`.d`; the other
  engines use the equivalent mzML (Astral) or native `.d` — the same underlying
  scans. Native reading is itself a andes capability, not a parameter.
- **timsTOF raw discrimination.** andes leads the timsTOF IDs but with a
  *weaker* raw target/decoy ratio (1.07 vs a comparison search engine 1.31 / the reference engine 1.48) — more of
...
- **the reference engine memory / decoys.** the reference engine DDA+ on the doubled (target+decoy)
...
See the user manual ([`../../DOCS.md`](../../DOCS.md)) for the full andes CLI,

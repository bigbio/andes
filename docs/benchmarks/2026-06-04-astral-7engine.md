...
Head-to-head of **andes** (top-1 and `--chimeric`), **the reference engine 4.2** (DDA+),
**a comparison search engine 0.14.7**, **a comparison engine 2025.01**, **Java MS-GF+ v20240326**, and **ProSE**
...
> peptides. In the chimeric tier, andes `--chimeric` and the reference engine DDA+ are neck
> and neck (andes ahead on PSMs+peptides, the reference engine on proteins). andes is the
...
| Isotope error | −1 … +2 (see §6 for the a comparison engine exception) |
...
|---|---|---:|---:|---:|---:|
| the reference engine 4.2 (DDA+) | mzML | 7:17 | **5,836** | 73,909 | 25,594 |
| **andes** (`--chimeric`) | **`.raw`** | 4:06 | 5,199 | **74,830** | **26,993** |
| a comparison search engine 0.14.7 (chimera) | mzML | 4:50 | 5,059 | 32,091 | 21,397 |
...
| a comparison engine 2025.01 | mzML | 3:30 | 4,354 | 31,435 | 20,607 |

Notes:
- **andes** is the only engine reading Thermo `.raw` natively. a comparison search engine's generic
  release lacks the proprietary Thermo reader (it tries to parse `.raw` as mzML
  and fails), so a comparison search engine/the reference engine/a comparison engine/Java/ProSE all run on the mzML.
...
### the reference engine 4.2 (DDA+) — `astral_all5.sh` step, `configs/the reference engine-astral.params`
...
### a comparison search engine 0.14.7 — `astral_fix.sh`, `configs/a comparison search engine-astral.json`
...
### a comparison engine 2025.01 — `comet_astral.sh`
Runs from the OpenMS `openms-tools-thirdparty` image
(`/opt/OpenMS/thirdparty/a comparison engine/a comparison engine.exe`). Params generated with `a comparison engine -p` then
...
- **a comparison engine isotope error**: a comparison engine has no exact `−1..+2` option; `isotope_error = 4`
...
- **Java & ProSE feature sets** are hand-built (§4), not produced by a native
  ...
  run. The four engines that emit a native PIN (andes, the reference engine, a comparison search engine, a comparison engine) are
...
- **Chimeric FDP**: reversed-decoy TDC does not expose coincidental real-DB targets
  ...
  database (§8). The the reference engine DDA+ chimeric counts were not entrapment-checked here
  (andes's were) and remain TDC-only.

## 7. Reproduce

On the benchmark host (engines + data staged under `$BENCH`):

```bash
bash scripts/astral_all5.sh        # andes top1+chimeric, the reference engine, a comparison search engine, ProSE
bash scripts/comet_astral.sh       # a comparison engine
...
Environment: RHEL 9, 8 cores; Percolator 3.7.1 and ProSE/a comparison engine via
`ghcr.io/openms/openms-tools-thirdparty:latest`; a comparison search engine v0.14.7 and the reference engine 4.2

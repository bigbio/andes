...
echo "=== [4] a comparison search engine v0.14.7 chimera=true (mzML) $(date -Is) ==="
# a comparison search engine's generic release lacks the proprietary Thermo reader, so it runs on the mzML.
...
echo "=== [5] the reference engine 4.2 DDA+ data_type=3 (mzML) $(date -Is) ==="
# the reference engine runs on the mzML here (the ext/thermo Batmass-IO reader is not installed).

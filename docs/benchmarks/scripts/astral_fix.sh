...
FRAGGER=$BENCH/engines/the reference engine-env/share/the reference engine-4.2-0/the reference engine-4.2/the reference engine-4.2.jar
a comparison search engine=$BENCH/engines/a comparison search engine-v0.14.7-x86_64-unknown-linux-gnu/a comparison search engine
SAGECFG=$BENCH/repo/msgf-rust/docs/benchmarks/configs/a comparison search engine-astral.json
FRGCFG=$BENCH/engines/the reference engine-cfg/astral_fragger.params
...
echo "################ ASTRAL FIX (a comparison search engine+the reference engine+ProSE) $(date -Is) ################"

echo "=== a comparison search engine v0.14.7 chimera=true (mzML) $(date -Is) ==="
# a comparison search engine's generic release lacks the proprietary Thermo reader, so it runs on the
# mzML (the benchmark doc records a comparison search engine as an mzML run).
sed -E "s#\"fasta\": *\"[^\"]*\"#\"fasta\": \"$FASTA\"#; s#\"output_directory\": *\"[^\"]*\"#\"output_directory\": \"$WORK/sage_astral\"#" $SAGECFG > $WORK/sage_astral.json
/usr/bin/time -v $a comparison search engine $WORK/sage_astral.json --write-pin -o $WORK/sage_astral $MZML > $RES/sage_astral.log 2>&1
echo "  exit=$?"; wallrss $RES/sage_astral.log
[ -f $WORK/sage_astral/results.a comparison search engine.pin ] && cp -f $WORK/sage_astral/results.a comparison search engine.pin $RES/sage_astral.pin
echo "  rows=$(($(wc -l < $RES/sage_astral.pin 2>/dev/null || echo 1)-1))"; perc sage_astral; count sage_astral

echo "=== the reference engine 4.2 DDA+ (mzML) $(date -Is) ==="

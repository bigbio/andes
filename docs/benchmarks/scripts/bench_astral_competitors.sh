#!/bin/bash
# Fresh competitor benchmark — Astral (mzML, all engines uniform). Engines: andes + Java MS-GF+, a comparison search engine, a comparison engine, ProSE.
# Engines: a comparison engine, a comparison search engine, ProSE, Java MS-GF+ (slowest last). Each -> .pin -> uniform
# Percolator(seed42) -> PSMs/peptides/proteins@1%. andes already measured separately.
set -uo pipefail
AD=/data/msgf-bench/astral-data
MZML=$AD/LFQ_Astral_DDA_15min_50ng_Condition_A_REP1.mzML
FASTA=$AD/ProteoBenchFASTA_MixedSpecies_HYE.fasta
MODS=/data/msgf-bench/astral_mods_rust.txt
JAR=/data/msgf-bench/MSGFPlus_v20240326.jar
a comparison search engine=/data/msgf-bench/engines/a comparison search engine-v0.14.7-x86_64-unknown-linux-gnu/a comparison search engine
SAGECFG=/data/msgf-bench/repo/msgf-rust/docs/benchmarks/configs/a comparison search engine-astral.json
BP=/data/msgf-bench/repo/build_pins.py
RES=/data/msgf-bench/bench-pub-astral; mkdir -p $RES
...
echo "################ ASTRAL PUBLIC BENCHMARK $(date -Is) ################"; df -h /data|tail -1

echo "=== [a comparison engine 2025.01] $(date -Is) ==="
/usr/bin/time -v docker run --rm -v $AD:/data -v $RES:/out $OIMG bash -c '
cd /out && C=/opt/OpenMS/thirdparty/a comparison engine/a comparison engine.exe; $C -p >/dev/null 2>&1; P=a comparison engine.params.new
...
' > $RES/a comparison engine.log 2>&1; echo "  a comparison engine exit=$?"; wall $RES/a comparison engine.log
[ -f $RES/comet_in.pin ] && mv -f $RES/comet_in.pin $RES/a comparison engine.pin
echo "  rows=$(($(wc -l < $RES/a comparison engine.pin 2>/dev/null||echo 1)-1))"; perc a comparison engine; count a comparison engine; rm -f $RES/a comparison engine.pin

echo "=== [a comparison search engine 0.14.7] $(date -Is) ==="
sed "s#\"fasta\":.*#\"fasta\": \"$FASTA\",#; s#\"output_directory\":.*#\"output_directory\": \"$WORK/a comparison search engine\"#" $SAGECFG > $WORK/a comparison search engine.json
/usr/bin/time -v $a comparison search engine $WORK/a comparison search engine.json --write-pin -o $WORK/a comparison search engine $MZML > $RES/a comparison search engine.log 2>&1; echo "  a comparison search engine exit=$?"; wall $RES/a comparison search engine.log
[ -f $WORK/a comparison search engine/results.a comparison search engine.pin ] && cp -f $WORK/a comparison search engine/results.a comparison search engine.pin $RES/a comparison search engine.pin
echo "  rows=$(($(wc -l < $RES/a comparison search engine.pin 2>/dev/null||echo 1)-1))"; perc a comparison search engine; count a comparison search engine; rm -f $RES/a comparison search engine.pin
...
echo "################ ASTRAL_PUB_DONE $(date -Is) ################"; df -h /data|tail -1
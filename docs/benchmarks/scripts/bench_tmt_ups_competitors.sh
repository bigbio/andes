...
# a comparison engine, Java MS-GF+, a comparison search engine, ProSE -> .pin -> uniform Percolator(seed42) ->
...
a comparison search engine=$B/engines/a comparison search engine-v0.14.7-x86_64-unknown-linux-gnu/a comparison search engine
...
" > $RES/a comparison engine.log 2>&1; echo "  a comparison engine exit=$?"; wall $RES/a comparison engine.log
  ...
  echo "  rows=$(($(wc -l < $RES/a comparison engine.pin 2>/dev/null||echo 1)-1))"; run_perc_count $RES a comparison engine; rm -f $RES/a comparison engine.pin; }
...
echo "=== [a comparison search engine] ==="
sed "s#SAGE_OUT_PLACEHOLDER#$W/a comparison search engine#g" $TD/a comparison search engine-a05058.json > $W/a comparison search engine.json
/usr/bin/time -v $a comparison search engine $W/a comparison search engine.json --write-pin -o $W/a comparison search engine $TD/$MZ > $RES/a comparison search engine.log 2>&1; echo "  a comparison search engine exit=$?"; wall $RES/a comparison search engine.log
[ -f $W/a comparison search engine/results.a comparison search engine.pin ] && cp -f $W/a comparison search engine/results.a comparison search engine.pin $RES/a comparison search engine.pin
echo "  rows=$(($(wc -l < $RES/a comparison search engine.pin 2>/dev/null||echo 1)-1))"; run_perc_count $RES a comparison search engine; rm -f $RES/a comparison search engine.pin
...
echo "################ TMT_PUB_DONE $(date -Is) ################"; df -h /data|tail -1
...
echo "=== [a comparison engine] (20ppm to match other engines) ==="
...
echo "  rows=$(($(wc -l < $RES/java.pin 2>/dev/null||echo 1)-1))"; run_perc_count $RES java; rm -f $RES/java.pin $W/java.mzid $W/java.tsv
echo "=== [a comparison search engine] ==="
sed "s#SAGE_OUT_PLACEHOLDER#$W/a comparison search engine#g" $DD/a comparison search engine-ups1.json > $W/a comparison search engine.json
/usr/bin/time -v $a comparison search engine $W/a comparison search engine.json --write-pin -o $W/a comparison search engine $DD/$MZ > $RES/a comparison search engine.log 2>&1; echo "  a comparison search engine exit=$?"; wall $RES/a comparison search engine.log
[ -f $W/a comparison search engine/results.a comparison search engine.pin ] && cp -f $W/a comparison search engine/results.a comparison search engine.pin $RES/a comparison search engine.pin
echo "  rows=$(($(wc -l < $RES/a comparison search engine.pin 2>/dev/null||echo 1)-1))"; run_perc_count $RES a comparison search engine; rm -f $RES/a comparison search engine.pin
echo "=== [ProSE] (0.4 Da) ==="
mkdir -p $RES/prose-out
...
python3 $BP prose $RES/prose-out/prose.idXML $RES/prose.pin > $RES/prose_build.log 2>&1
echo "  prose rows=$(($(wc -l < $RES/prose.pin 2>/dev/null||echo 1)-1))"; run_perc_count $RES prose; rm -f $RES/prose.pin
echo "################ UPS1_PUB_DONE $(date -Is) ################"; df -h /data|tail -1

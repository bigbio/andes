...
RES=/data/msgf-bench/repo/bench-astral; mkdir -p $RES
...
COMET_EXIT=$?
echo "  a comparison engine exit=$COMET_EXIT"; grep -E "Elapsed \(wall|Maximum resident" $RES/comet_astral.log | sed "s/^/  /"

#!/bin/bash
set -uo pipefail
TD=/data/msgf-bench/tmt-data
MZML=a05058.mzML
FASTA=PXD007683_UP000005640_UP000002311_reviewed.fasta
RES=/data/msgf-bench/repo/bench-tmt; mkdir -p $RES
OIMG=ghcr.io/openms/openms-tools-thirdparty:latest
PIMG=quay.io/biocontainers/percolator:3.7.1--h3b5f4bd_2
echo "################ a comparison engine TMT a05058 $(date -Is) ################"
/usr/bin/time -v docker run --rm -v $TD:/data -v $RES:/out $OIMG bash -c '
cd /out && C=/opt/OpenMS/thirdparty/a comparison engine/a comparison engine.exe
$C -p >/dev/null 2>&1
P=a comparison engine.params.new
sed -i -E "
...
COMET_EXIT=$?
echo "  a comparison engine exit=$COMET_EXIT"; grep -E "Elapsed \(wall|Maximum resident" $RES/comet_tmt.log | sed "s/^/  /"

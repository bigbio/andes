...
REPO=/data/msgf-bench/repo/msgf-rust
B="${ANDES:-$REPO/target/release/andes}"; [ -x "$B" ] || B="$REPO/target/release/cimas"; [ -x "$B" ] || B="$REPO/target/release/simas"
M=/data/msgf-bench/repo/msgf-rust/resources/ionstat/models.parquet
RES=/data/msgf-bench/repo/bench-entrapment; mkdir -p $RES

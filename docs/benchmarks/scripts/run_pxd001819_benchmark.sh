#!/bin/bash
# Benchmark: Baseline vs New MS-GF+ on PXD001819 (Yeast + UPS1 spike-in)
#
# Dataset: PXD001819 — LFQ, CID, LTQ Orbitrap Velos
#   Spectra: https://ftp.pride.ebi.ac.uk/pub/databases/pride/resources/proteomes/benchmarks/lfq/LTQOrbitrapVelos/PXD001819/
#   FASTA:   https://github.com/bigbio/quantms-test-datasets (databases/PXD001819_uniprot_yeast_ups.fasta)
#   SDRF:    https://github.com/bigbio/quantms-test-datasets (benchmarks/lfq/LTQOrbitrapVelos/PXD001819/)
#
# Search parameters extracted from SDRF:
#   Enzyme: Trypsin/P, Precursor: 5ppm, Fragment: 0.8Da, CID
#   Fixed: Carbamidomethyl (C), Variable: Oxidation (M), Acetyl (Protein N-term)
#
# Usage:
#   bash run_pxd001819_benchmark.sh              # Run both baseline + new
#   bash run_pxd001819_benchmark.sh new-only      # Run only the new JAR
#   bash run_pxd001819_benchmark.sh baseline-only  # Run only the baseline JAR
#
set -euo pipefail
...
# Search parameters from SDRF: Trypsin/P (-e 1), CID (-m 0), LTQ Orbitrap (-inst 0)
# Precursor: 5ppm, Fragment: 0.8Da (handled by -inst 0 default)
...
for f in "$MZML" "$FASTA" "$MODS"; do
    if [ ! -f "$f" ]; then
        ...
        echo "Run download_data.sh first."
        exit 1
    fi
done
...
echo "========== PXD001819 BENCHMARK (Yeast + UPS1) =========="
echo "Date: $(date)"
...
echo "mzML: $(basename "$MZML") ($(du -sh "$MZML" | cut -f1))"

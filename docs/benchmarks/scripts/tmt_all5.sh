...
SAGECFG=$TD/a comparison search engine-a05058.json
...
echo "=== [3] a comparison search engine v0.14.7 (mzML) $(date -Is) ==="
sed "s#SAGE_OUT_PLACEHOLDER#$WORK/sage_tmt#g" $SAGECFG > $WORK/sage_tmt.json
/usr/bin/time -v $a comparison search engine $WORK/sage_tmt.json --write-pin -o $WORK/sage_tmt $MZML > $RES/sage_tmt.log 2>&1
echo "  exit=$?"; wallrss $RES/sage_tmt.log
[ -f $WORK/sage_tmt/results.a comparison search engine.pin ] && cp -f $WORK/sage_tmt/results.a comparison search engine.pin $RES/sage_tmt.pin
echo "  rows=$(($(wc -l < $RES/sage_tmt.pin 2>/dev/null || echo 1)-1))"; perc sage_tmt; count sage_tmt

echo "=== [4] the reference engine 4.2 (mzML) $(date -Is) ==="

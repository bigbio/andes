...
Run on the self-hosted bench VM (`/data/msgf-bench`). These scripts implement
...
- 53+ GB free under `/data` for MSNet RAW staging
...
bash /data/msgf-bench/phase_v_train_intensity.sh

# 3. Run the rank vs strong A/B gate (Astral + TMT + optional PTM)
bash /data/msgf-bench/phase_v_strong_score_gate.sh

# 4. Summarize pass/fail
python3 /data/msgf-bench/summarize_phase_v_gate.py /data/msgf-bench/phase-v-*
```
...
nohup bash /data/msgf-bench/phase_v_cid_decision_pipeline.sh \
  > /data/msgf-bench/phase_v_cid_decision.log 2>&1 &

# Or stepwise:
bash /data/msgf-bench/phase_v_cid_train_models.sh
bash /data/msgf-bench/phase_v_cid_benchmark.sh
python3 /data/msgf-bench/summarize_phase_v_cid_benchmark.py /data/msgf-bench/cid-bench-*
```
...
```bash
export PTM_MZML=/data/msgf-bench/ptm-data/sample.mzML
export PTM_FASTA=/data/msgf-bench/ptm-data/database.fasta
export PTM_ENTRAP=/data/msgf-bench/ptm-data/entrapment.fasta
export PTM_MODS=/data/msgf-bench/ptm-data/mods.txt

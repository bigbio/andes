...
#
# Usage:
#   bash /data/msgf-bench/phase_v_cid_benchmark.sh
#   DATASETS=PXD007683 bash ...   # run one dataset only
set -uo pipefail

cd /data/msgf-bench

REPO="${REPO:-/data/msgf-bench/repo/msgf-rust}"
BIN="${BIN:-$REPO/target/release/andes}"
BUNDLED="${BUNDLED:-$REPO/resources/ionstat/models.parquet}"
INTENSITY_MODEL="${INTENSITY_MODEL:-/data/msnet/intensity_model_cid.parquet}"
SCORING_STORE="${SCORING_STORE:-/data/msnet/models_cid_msnet.parquet}"
OUT="${OUT:-/data/msgf-bench/cid-bench-$(date +%Y%m%d-%H%M)}"
DATASETS="${DATASETS:-PXD016999,PXD007683,PXD001819}"

FRAGGER="${FRAGGER:-/data/msgf-bench/engines/msfragger-env/share/msfragger-4.2-0/MSFragger-4.2/MSFragger-4.2.jar}"
TRFP="${TRFP:-/data/.conda/envs/nextflow/bin/ThermoRawFileParser}"
COMET_RUN="${COMET_RUN:-/data/msgf-bench/comet_cid_run.sh}"
...
process_file_baseline() {
  ...
  rm -f "$mzml"
  echo "  done $tag free=$(df -h /data | tail -1 | awk '{print $4}')"
}

run_pxd016999() {
  local ds=PXD016999
  local base=/data/msgf-bench/pxd016999
...
run_pxd007683() {
  local ds=PXD007683
  local base=/data/msgf-bench/tmt-data
  ...
  [ -f "$frg" ] || frg=/data/msgf-bench/repo/msgf-rust/docs/benchmarks/configs/msfragger-a05058.params
  ANDES_RAWS=()
  local entries=()
...
python3 /data/msgf-bench/summarize_phase_v_cid_benchmark.py "$OUT" || true
echo "OUT=$OUT"

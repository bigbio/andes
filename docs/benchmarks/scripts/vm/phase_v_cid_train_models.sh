...
#
# Output:
#   /data/msnet/intensity_model_cid.parquet
#   /data/msnet/models_cid_msnet.parquet
set -euo pipefail

cd /data/msnet
mkdir -p raw partial flat

SCRIPT_DIR="${SCRIPT_DIR:-/data/msgf-bench}"
...
AGG="${AGG:-/data/msnet/msnet_intensity_agg.py}"
FLAT="${FLAT:-/data/msnet/msnet_to_flat.py}"
INTENSITY_OUT="${INTENSITY_OUT:-/data/msnet/intensity_model_cid.parquet}"
SCORING_OUT="${SCORING_OUT:-/data/msnet/models_cid_msnet.parquet}"
...
stage_cid_one() {
  ...
  echo "[$id] free=$(df -h /data | tail -1 | awk '{print $4}')"
}

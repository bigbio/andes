...
INTENSITY_MODEL="${INTENSITY_MODEL:-/data/msnet/intensity_model.parquet}"
OUT="${OUT:-/data/msgf-bench/phase-v-$(date +%Y%m%d-%H%M)}"
SKIP_PTM="${SKIP_PTM:-0}"
...
column -t -s $'\t' "$SUMMARY"
python3 /data/msgf-bench/summarize_phase_v_gate.py "$OUT"
echo "OUT=$OUT"

...
3. **Field-beating** — ≥ MSFragger/Sage on confident PSMs at **1% true entrapment-FDP** across Astral (high-res HCD), TMT (low-res CID), UPS1 (LFQ), and HLA.
...
- **Local repo** (`/Users/user/work/msgfplus-workspace/msgf-rust`): all Rust code changes (F2, F3, A1-packing, E3, W1, W3), the gate script, the consolidated PR. Build the release binary here, ship to HPC/VM.
- **HPC** (`/cluster/andes-training`, hpc-cluster skill): harvesting (F1 python edit), training (`cluster_slug.sh`/`submit_array.sh`, fixed), `andes-bin` (rebuild from main after each code change), stores, flats, seed-models.parquet, assembler.
- **VM** (`bench-vm`, `~/.ssh/<key>`): the benchmark (`ab_a05058.sh`, Percolator 3.7.1, `compute_entrapment_fdp.py`, `TMPDIR=/data/msgf-bench/abtmp`).
- **Provenance rule:** one binary commit → rsync crates + `cargo build --release -p andes` on HPC AND VM; never let the two hosts drift (experiment-hygiene memory).
...
### 1a. Bug B durable fix + harness hygiene  *(HPC, small)*
...
### 1b. F1 — dataset-wide q-value labels in flats  *(HPC python, low effort, highest quality lever)*
- Edit `mzml_pepxml_to_flat.py`: replace the per-PSM `expect ≤ 0.01` cut with a **target/decoy TDC q-value ≤ 0.01** (MSFragger emits decoys; mirror `crates/model-train/src/labeled.rs` monotone q-walk + conservative tie-bucketing). Keep rank-1, charge≥… , peaks≥10.
...
- **C1 — cross-species pooling**: add E. coli (PXD018176, 242k) + yeast flats *already on HPC* into `hcd_qexactive_tryp`, weight-swept (0.25/0.5/1.0) → Astral PSMs@1%. Fills high-charge/high-mass tails. (F2 makes this safe — without it, heterogeneous data re-triggers the noise inflation.)
...
- **Full benchmark** vs MSFragger + Sage on Astral/TMT/UPS1 (+ HLA) at 1% true entrapment-FDP, uniform Percolator, fresh wall + uniform parsimony.
...
- **B4** self-labeling with andes once it leads a regime (removes the MSFragger-ceiling + independence concern).

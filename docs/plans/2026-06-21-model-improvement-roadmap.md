...
**Goal:** the final fully-own andes model set that BEATS MSFragger/Sage on confident PSMs at 1% true entrapment-FDP across regimes.
...
4. **Beat-the-teacher ceiling** — *harvesting (strategic).* Training on MSFragger's labels caps the ceiling at "what MSFragger finds" + bakes in its blind spots → at odds with *beating* it, and an independence concern. Long-term fix: self-label with andes once it leads (the in-repo `bootstrap_labels` TDC path).
...
- **A1. Retrain + repack the models WITH the GBDT blob columns** (architecture #1). Turns RawScore/RichIonLLR/frag-LLR battery from zeros to live. Medium effort (HPC retrain + repack, no engine change). Validate: non-zero PIN columns + PSMs@1% entrapment-FDP vs shipped. **This is the first thing to measure.**
...
- **C1. Cross-species pooling** (cross-species #1) — E. coli (PXD018176, 242k) + yeast flats *already on HPC* into hcd_qexactive_tryp; weight-swept; A/B Astral. Fills high-charge/high-mass tails.

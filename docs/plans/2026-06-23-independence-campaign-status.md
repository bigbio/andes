...
Proven pipeline (per regime): public corpus → the reference engine-labeled search → flats → `retrain_slug.sh <slug> <seed>` (geometry from bundle seed) → own-trained store. Validated by `hcd_astral_tryp` (2h40m, field-beating +23–50% vs the reference engine/a comparison engine/MS-GF+).

## Wave 1 — TRAIN ready-flat regimes (HPC array <id>, in flight)
...
HARVESTING NOW (HPC array <id>): hcd_qexactive_tryp_itraqphospho (PXD006380), cid_lowres_tryp_phosphorylation (PXD001428 trypsin arm), cid_lowres_lysn_phosphorylation (PXD001114 LysargiNase proxy), hcd_highres_tryp_phosphorylation (PXD009449). ETA 3-8h.
...
- Astral (hcd_astral_tryp): field-beating +23-50% vs the reference engine/a comparison engine/MS-GF+.
...
Own-trained v1 numbers (std 1% FDR): Astral top-1 39,506 (was 36,782, +7.4%!) / chimeric 68,244 (+117% vs field); UPS1 chimeric 18,732 (+4.8%, leads field) / top-1 17,129 (within 1% Java, README framing holds). Beats Java(26,542/-/17,305)+a comparison engine(31,435/-/15,809) on Astral+UPS. Entrapment-FDP honest (0.9-1.2%).
...
Detection-fixed binary + partitioned v1 bundle, /data/msgf-bench/v1-definitive/. Recovered by counting the Percolator outputs (agent crashed before reporting).
...
★ FINAL v1 table (q<=0.01 std FASTA, own models, leads all 3):
  ...
  Java MS-GF+:    26,542 / 11,555 / 17,305    a comparison engine: 31,435 / 10,876 / 15,809
...
★ DO NOT claim a +11.7% Astral improvement. The andes-v1(mix-max/revCat) vs Java/a comparison engine(old -Y/plain) table also MIXES methodologies -> not strictly fair.
...
FIX: FINAL fair benchmark a2020b68 = andes-v1 on the EXACT old methodology (plain FASTA + andes decoys + percolator -Y, matching the reused Java 26,542/a comparison engine 31,435) -> publishable table. Expect andes-v1 ~= old 36,782 (model A/B tie) + still leads field. NO methodology drift this time (no revCat, no mix-max).
...
## ★ INDEPENDENCE INTEGRITY VERIFIED (value-level, 2026-06-24) — pure-own claim HOLDS
Concern: retrain_slug.sh logs 'trained table rows: 981 seed table rows: 981' for self-seeded regimes (vs hcd_astral_tryp 1535/0) -> feared thin-flat regimes inherited MS-GF+ seed tables. VERIFIED by value comparison (md5 of each table row's value columns, trained store vs the 39-slug MS-GF+ seed $B/cand/_run/resources/models.parquet): cid_lowres_lysc(1 flat)/argc(3)/cid_lowres_tryp_phosphorylation/hcd_qexactive_tryp_phosphorylation/_tmt = 0% identical-to-seed -> genuinely RETRAINED (own data + pseudocount backoff, NOT inherited). The '981 seed rows' was a ROW-COUNT match (partition structure), NOT value inheritance. => v1 is MS-GF+-free at the VALUE level (stronger than the build agent's source-level check). CAVEAT: 1-3-flat regimes (lysc/argc/gluc) are own but WEAKLY trained (pseudocount-dominated) -> quality follow-up, not an independence issue.
Wave-2 train DONE -> 19 own-trained (added cid_lowres_tryp_phosphorylation + hcd_highres_tryp_phosphorylation; both 0% identical-to-seed = own).

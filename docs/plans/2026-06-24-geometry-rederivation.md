...
Prior art (sourced): the **top-150 peak cap** is field-standard (a comparison search engine
`max_peaks`=150, the reference engine `use_topN_peaks`=150, *verbatim*); coarse m/z
...
1. `max_rank ∈ {100,125,150,200}` (cheapest; expect flat ≈150, a comparison search engine-corroborated)

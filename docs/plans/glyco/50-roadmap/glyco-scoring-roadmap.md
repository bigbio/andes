...
andes beats an open-source glyco engine (253/97 @1% FDR, deterministic) but is at ~48% of
the reference engine's curated truth (523). The gap is **not** FDR (costs 6 scans), **not**
transfer (ceiling +52), and **not** candidate generation (~80% of true backbones
are generated). The isolation experiment proved that simply enlarging the
candidate set (full glycan list, higher top-k) *lowers* @1% — so the binding
constraint is **discriminative scoring**: for ~106–300 truth scans the true
backbone is generated but a wrong one out-ranks it.

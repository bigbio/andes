...
|---|---|---|---|---|
| **GI-1** | ~50% of andes glyco PSMs are **de-novo mass residuals**, not enumerated glycan compositions | 700,705 target rows: 352,475 `IsGlycanDb=1` vs **348,230 de-novo** (35,882 distinct "GlycanMass" up to 10,698 Da) | **✅ FIXED** (087e2577) | `select_emitted_hits` filters to `glycan.is_some()` before the top-1 collapse (default; `ANDES_GLYCO_DENOVO=1` = diagnostic). |
| **GI-2** | Glyco **features do not separate target from (reversed-peptide) decoy** | target vs decoy means IDENTICAL: GlycanMass 1692.8≈1698.8, YLadder 0.043≈0.044, CoreY 0.978≈0.996 | **DESIGNED — research lever** | Naive "copy decoy to all columns" is a NO-OP (analysis below): Oxonium/CoreY/GlycanMass are spectrum/backbone-level and cannot discriminate a glycan on a fixed spectrum. Real fix = a **composition-conditioned SIALIC-oxonium feature** (NeuAc 274.09/292.10, NeuGc — andes's oxonium panel is HexNAc-only today) + optional isobaric-composition decoys. Do on the clean GI-1/GI-3 PIN. |
| **GI-3** | andes's glycan **DB has coverage gaps** vs the reference engine | 27/123 (22%) the reference engine-assigned glycans absent: 8 small <892 Da (HexNAc, HexNAc2, HexNAc2Hex1-2 = paucimannose/truncated) + 19 large (1458–4028) | **✅ FIXED** (13dfc5dd) | Added paucimannose block (HexNAc 1-2, Hex 0-2, ±Fuc); all 8 small gaps now enumerated. The 19 large are already in the full DB (unmatched in this file, not a gap). |

# Fundamental deterministic candidate-scoring improvements for Andes

**2026-06-04** — synthesis of the internal idea docs (`internal-docs/2026-06-04-andes-
...
**Frame (the Andes identity):** Andes is a *deterministic candidate-generation-and-
...
Andes can match at true ppm and add a **mass-error-consistency** term + an **averagine
...
precursor exist with the right isotope envelope — DDA+). Andes reads `.raw`/`.d` natively,
...
## Axis 2 — Relation to other candidates (the Andes differentiator)
...
discovery, not N). "Weak + weak + same-latent constraint = strong." Needs only Andes's
...
NOT Andes's job (keep in the separate rescoring layer): transformer/contrastive rerankers,

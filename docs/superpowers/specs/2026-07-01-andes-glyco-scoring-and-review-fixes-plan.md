...
- **P1.3 Evaluate:** top-1 accuracy on the 523 truth scans (target 83 → toward 154), via the local labels — no HPC needed.
...
- HPC multi-dataset harvest (PXD005565 dual a glyco search engine+a commercial glyco engine, PXD030622, PXD025859) → train a general glyco model the andes way. Resource-heavy; gated on prototype success.

## Evaluation harness (reusable, already built)
- `glyco_probe` (backbone find-rate, residue-convention) · `findrate.py` (peptide-level) · true/false @1% feature-separation script · glycan-coverage diagnostic · VM `/data/msgf-bench/glyco_bench/` staged (mzML+FASTA+truth) · Percolator via `run_percolator_docker.sh` + top-1/scan collapse.
...
3. Gate before Phase 4: only harvest on HPC if the local prototype lifts top-1 accuracy AND true-FDP materially.

...
impl AminoAcidSet {
    ...
    // -----------------------------------------------------------------------
    // Phase 6 / Task 5 GF helpers — mirroring Java AminoAcidSet.getAAList,
    // getPeptideCleavageCredit/Penalty, getNeighboringAACleavageCredit/Penalty,
    // and getProbCleavageSites.
    // -----------------------------------------------------------------------

    /// All amino acid variants valid at `location`. Mirrors Java
    /// `AminoAcidSet.getAAList(Location)`.
    ///
    ...
    /// cleavage site. Java: `AminoAcidSet.getPeptideCleavageCredit()`.
    ///
    /// Computed by Java as `round(log(efficiency / probCleavageSites))`.
    /// For Phase 6 we hard-code the Java default used when no enzyme is
    /// registered (both efficiency and probCleavageSites are 0, credit = 0).
    /// Callers that have a real enzyme should use `register_enzyme` first; for
    /// the graph construction where credit/penalty are used directly in
    /// Phase 6 Task 5b, we expose the stored values set by `register_enzyme`.
    ///
    /// Default: `0` (no enzyme registered). Java default before `registerEnzyme` = 0.
    ...
    /// at a cleavage site. Java: `AminoAcidSet.getProbCleavageSites()` which is
    /// set by `registerEnzyme`.
    ///
    ...
    /// efficiency values. Mirrors Java `AminoAcidSet.registerEnzyme(Enzyme)`.
    ///
    /// Formula (Java):
    /// ```text
...
mod tests {
    ...
    fn peptide_cleavage_credit_default_is_zero() {
        let set = AminoAcidSetBuilder::new_standard().build().unwrap();
        // Default before register_enzyme is 0, not 1. Java also defaults to 0.

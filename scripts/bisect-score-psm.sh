...
#
# - Builds simas at the current commit
...
# `git bisect run` logs.
if ! cargo build --release --bin simas 2>&1 | tail -5; then
    echo "[$SHA] build failed — skip"
    echo "$SHA,BUILD_FAIL" >> "$TRACE_CSV"
    exit 125
fi

BIN="$REPO_ROOT/rust/target/release/simas"
rm -f "$PIN_OUT"
...
if ! "$BIN" \
    ...
    echo "[$SHA] simas run failed — skip"
